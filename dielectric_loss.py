...
def process_directory(root_folder, mapping):
    ...
    output_root = create_output_folder(root_folder, f"Dielectric_Loss_Result_{now}")
    ...
    for dirpath, _, files in os.walk(root_folder):
        for file in files:
            if file.endswith(".csv"):
                ...
                result = process_dielectric_loss(df)
                if result is None:
                    continue
